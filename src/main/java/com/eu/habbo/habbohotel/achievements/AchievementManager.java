...
import com.eu.habbo.messages.outgoing.achievements.AchievementMessageComposer;
import com.eu.habbo.messages.outgoing.achievements.HabboAchievementNotificationMessageComposer;
import com.eu.habbo.messages.outgoing.achievements.talenttrack.TalentLevelUpMessageComposer;
...
public class AchievementManager {
    ...
    public static void progressAchievement(Habbo habbo, Achievement achievement, int amount) {
        ...
        if (newLevel == null ||
            ...
            habbo.getClient().sendResponse(new AchievementMessageComposer(habbo, achievement));
        } else {
            ...
            habbo.getClient().sendResponse(new AchievementMessageComposer(habbo, achievement));
            habbo.getClient().sendResponse(new HabboAchievementNotificationMessageComposer(habbo, achievement));
            ...
            if (badge.getSlot() > 0) {
                if (habbo.getHabboInfo().getCurrentRoom() != null) {
                    habbo.getHabboInfo().getCurrentRoom().sendComposer(new UserBadgesMessageComposer(habbo.getInventory().getBadgesComponent().getWearingBadges(), habbo.getHabboInfo().getId()).compose());
                }
            }

            habbo.getClient().sendResponse(new UnseenItemsMessageComposer(badge.getId(), UnseenItemsMessageComposer.AddHabboItemCategory.BADGE));
                    ...
                    if (level != null) {
                        if (level.items != null && !level.items.isEmpty()) {
                            for (Item item : level.items) {
                                ...
                                habbo.getClient().sendResponse(new UnseenItemsMessageComposer(rewardItem));
                            ...
                            for (String badge : level.badges) {
                                if (!badge.isEmpty()) {
                                    if (!habbo.getInventory().getBadgesComponent().hasBadge(badge)) {
                                        ...
                                        habbo.getClient().sendResponse(new BadgeReceivedMessageComposer(b));
                                    }
                                }
                            }
                        }
                        ...
                        habbo.getClient().sendResponse(new TalentLevelUpMessageComposer(type, level));
                    }
                }
            }
