...
import com.eu.habbo.messages.outgoing.generic.ClubGiftNotificationMessageComposer;
...
public class SubscriptionHabboClub extends Subscription {
    ...
    public void onCreated() {
        ...
        if (habbo != null && habbo.getClient() != null) {
            ...
            if (habbo.getHabboStats().getRemainingClubGifts() > 0) {
                habbo.getClient().sendResponse(new ClubGiftNotificationMessageComposer(habbo.getHabboStats().getRemainingClubGifts()));
            }

            if ((Emulator.getIntUnixTimestamp() - habbo.getHabboStats().hcMessageLastModified) < 60) {
                Emulator.getThreading().run(() -> {
                    habbo.getClient().sendResponse(new ScrSendUserInfoMessageComposer(habbo));
                ...
                habbo.getClient().sendResponse(new ScrSendUserInfoMessageComposer(habbo, SubscriptionHabboClub.HABBO_CLUB, ScrSendUserInfoMessageComposer.RESPONSE_TYPE_NORMAL));
                habbo.getClient().sendResponse(new UserRightsMessageComposer(habbo));
            }
        }
    }
     ...
     * @return ScrSendKickbackInfoMessageComposer
     */
