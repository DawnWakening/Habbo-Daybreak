...
import com.eu.habbo.messages.outgoing.friends.NewFriendRequestMessageComposer;
import com.eu.habbo.messages.outgoing.friends.MessengerErrorMessageComposer;
...
public class RequestFriendMessageEvent extends MessageHandler {
    private static final Logger LOGGER = LoggerFactory.getLogger(RequestFriendMessageEvent.class);
    ...
    public void handle() throws Exception {
        ...
        if (targetHabbo == null) {
            this.client.sendResponse(new MessengerErrorMessageComposer(MessengerErrorMessageComposer.TARGET_NOT_FOUND));
            return;
        }
        ...
        if (targetBlocksFriendRequests) {
            this.client.sendResponse(new MessengerErrorMessageComposer(MessengerErrorMessageComposer.TARGET_NOT_ACCEPTING_REQUESTS));
            return;
        }
        ...
        if (this.client.getHabbo().getMessenger().getFriends().values().size() >= this.client.getHabbo().getHabboStats().maxFriends && !this.client.getHabbo().hasPermission("acc_infinite_friends")) {
            this.client.sendResponse(new MessengerErrorMessageComposer(MessengerErrorMessageComposer.FRIEND_LIST_OWN_FULL));
            return;
        }
        ...
        if (targetHabbo.getMessenger().getFriends().values().size() >= targetHabbo.getHabboStats().maxFriends && !targetHabbo.hasPermission("acc_infinite_friends")) {
            this.client.sendResponse(new MessengerErrorMessageComposer(MessengerErrorMessageComposer.FRIEND_LIST_TARGET_FULL));
            return;
        }
        ...
        if (Emulator.getPluginManager().fireEvent(new UserRequestFriendshipEvent(this.client.getHabbo(), username, targetHabbo)).isCancelled()) {
            this.client.sendResponse(new MessengerErrorMessageComposer(2));
            return;
        }
