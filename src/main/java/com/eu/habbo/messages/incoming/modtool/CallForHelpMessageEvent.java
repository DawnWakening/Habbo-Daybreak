...
public class CallForHelpMessageEvent extends MessageHandler {
    @Override
    public void handle() throws Exception {
        ...
        this.packet.readInt(); // messageCount - not used
                            ...
                            Emulator.getThreading().run(() -> {
                                if (issue.state == ModToolTicketState.OPEN) {
                                    if (cfhTopic.action == CfhActionType.AUTO_IGNORE) {
                                        if (CallForHelpMessageEvent.this.client.getHabbo().getHabboStats().ignoreUser(CallForHelpMessageEvent.this.client, reported.getHabboInfo().getId())) {
                                            CallForHelpMessageEvent.this.client.sendResponse(new IgnoreResultMessageComposer(reported, IgnoreResultMessageComposer.IGNORED));
                                        }
                                    }

                                    CallForHelpMessageEvent.this.client.sendResponse(new IssueCloseNotificationMessageComposer(cfhTopic.reply).compose());
            ...
            if (cfhTopic != null) {
                if (cfhTopic.action != CfhActionType.MODS) {
                    Emulator.getThreading().run(() -> {
                        if (issue.state == ModToolTicketState.OPEN) {
                            if (cfhTopic.action == CfhActionType.AUTO_IGNORE) {
                                if (CallForHelpMessageEvent.this.client.getHabbo().getHabboStats().ignoreUser(CallForHelpMessageEvent.this.client, issue.reportedId)) {
