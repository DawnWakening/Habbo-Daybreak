...
public class StalkFriendEvent extends MessageHandler {
    ...
    public void handle() throws Exception {
        ...
        if (buddy == null) {
            this.client.sendResponse(new FollowFriendFailedMessageComposer(FollowFriendFailedMessageComposer.NOT_IN_FRIEND_LIST));
            return;
        }
        ...
        if (habbo == null || !habbo.isOnline()) {
            this.client.sendResponse(new FollowFriendFailedMessageComposer(FollowFriendFailedMessageComposer.FRIEND_OFFLINE));
            return;
        }

        if (habbo.getHabboStats().blockFollowing && !this.client.getHabbo().hasPermission("acc_can_stalk")) {
            this.client.sendResponse(new FollowFriendFailedMessageComposer(FollowFriendFailedMessageComposer.FRIEND_BLOCKED_STALKING));
            return;
        }

        if (habbo.getHabboInfo().getCurrentRoom() == null) {
            this.client.sendResponse(new FollowFriendFailedMessageComposer(FollowFriendFailedMessageComposer.FRIEND_NOT_IN_ROOM));
            return;
        }
