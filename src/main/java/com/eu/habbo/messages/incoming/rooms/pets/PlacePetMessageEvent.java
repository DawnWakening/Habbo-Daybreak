...
import com.eu.habbo.messages.outgoing.generic.alerts.PetPlacingErrorMessageComposer;
import com.eu.habbo.messages.outgoing.inventory.PetRemovedFromInventoryMessageComposer;
import com.eu.habbo.messages.outgoing.rooms.pets.PetFigureUpdateMessageComposer;

public class PlacePetMessageEvent extends MessageHandler {
    @Override
    public void handle() throws Exception {
        ...
        if (this.client.getHabbo().getHabboInfo().getId() != room.getOwnerId() && !room.isAllowPets() && !(this.client.getHabbo().hasPermission(Permission.ACC_ANYROOMOWNER) || this.client.getHabbo().hasPermission(Permission.ACC_PLACEFURNI))) {
            this.client.sendResponse(new PetPlacingErrorMessageComposer(PetPlacingErrorMessageComposer.ROOM_ERROR_PETS_FORBIDDEN_IN_FLAT));
            return;
        }
        ...
        if (room.getCurrentPets().size() >= Room.MAXIMUM_PETS && !this.client.getHabbo().hasPermission(Permission.ACC_UNLIMITED_PETS)) {
            this.client.sendResponse(new PetPlacingErrorMessageComposer(PetPlacingErrorMessageComposer.ROOM_ERROR_MAX_PETS));
            return;
        }
        ...
        if ((x == 0 && y == 0) || !room.isOwner(this.client.getHabbo())) {
            ...
            if (tile == null || !tile.isWalkable()) {
                this.client.sendResponse(new PetPlacingErrorMessageComposer(PetPlacingErrorMessageComposer.ROOM_ERROR_PETS_NO_FREE_TILES));
            }
        ...
        if (tile == null || !tile.isWalkable() || !tile.getAllowStack()) {
            this.client.sendResponse(new PetPlacingErrorMessageComposer(PetPlacingErrorMessageComposer.ROOM_ERROR_PETS_SELECTED_TILE_NOT_FREE));
            return;
        }
        ...
        room.sendComposer(new PetFigureUpdateMessageComposer(pet).compose());
