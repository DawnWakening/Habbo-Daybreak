...
import com.eu.habbo.messages.outgoing.trading.TradingOpenFailedMessageComposer;
            ...
            if (room != null) {
                if (userId >= 0 && userId != this.client.getHabbo().getRoomUnit().getId()) {
                    ...
                    if (!RoomTrade.TRADING_ENABLED && !tradeAnywhere) {
                        this.client.sendResponse(new TradingOpenFailedMessageComposer(TradingOpenFailedMessageComposer.HOTEL_TRADING_NOT_ALLOWED));
                        return;
                    }

                    if ((room.getTradeMode() == 0 || (room.getTradeMode() == 1 && this.client.getHabbo().getHabboInfo().getId() != room.getOwnerId())) && !tradeAnywhere) {
                        this.client.sendResponse(new TradingOpenFailedMessageComposer(TradingOpenFailedMessageComposer.ROOM_TRADING_NOT_ALLOWED));
                        return;
                    }
                    ...
                    if (!this.client.getHabbo().getHabboStats().allowTrade()) {
                        this.client.sendResponse(new TradingOpenFailedMessageComposer(TradingOpenFailedMessageComposer.YOU_TRADING_OFF));
                        return;
                    }
                    ...
                    if (!targetUser.getHabboStats().allowTrade()) {
                        this.client.sendResponse(new TradingOpenFailedMessageComposer(TradingOpenFailedMessageComposer.TARGET_TRADING_NOT_ALLOWED, targetUser.getHabboInfo().getUsername()));
                        return;
                    }
