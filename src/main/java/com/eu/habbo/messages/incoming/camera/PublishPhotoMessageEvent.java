...
public class PublishPhotoMessageEvent extends MessageHandler {
    private static final Logger LOGGER = LoggerFactory.getLogger(PublishPhotoMessageEvent.class);
        ...
        if (cooldownLeft == 0) {
            ...
            if (!Emulator.getPluginManager().fireEvent(publishPictureEvent).isCancelled()) {
                try (Connection connection = Emulator.getDatabase().getDataSource().getConnection(); PreparedStatement statement = connection.prepareStatement("INSERT INTO camera_web (user_id, room_id, timestamp, url) VALUES (?, ?, ?, ?)")) {
                    ...
                    this.client.getHabbo().givePoints(PublishPhotoMessageEvent.CAMERA_PUBLISH_POINTS_TYPE, -PublishPhotoMessageEvent.CAMERA_PUBLISH_POINTS);
