...
import com.eu.habbo.messages.outgoing.navigator.UserEventCatsMessageComposer;
import com.eu.habbo.messages.outgoing.rooms.promotions.RoomPromotionMessageComposer;

public class PurchaseRoomAdMessageEvent extends MessageHandler {
    ...
    public void handle() throws Exception {
        ...
        if (UserEventCatsMessageComposer.CATEGORIES.stream().noneMatch(c -> c.getId() == categoryId))
            return;
        ...
        if (item != null) {
            if (this.client.getHabbo().getHabboInfo().canBuy(item)) {
                ...
                if (room.isPromoted()) {
                    ...
                    if (!this.client.getHabbo().getInventory().getBadgesComponent().hasBadge(PurchaseRoomAdMessageEvent.ROOM_PROMOTION_BADGE)) {
                        this.client.getHabbo().addBadge(PurchaseRoomAdMessageEvent.ROOM_PROMOTION_BADGE);
                    }
                } else {
