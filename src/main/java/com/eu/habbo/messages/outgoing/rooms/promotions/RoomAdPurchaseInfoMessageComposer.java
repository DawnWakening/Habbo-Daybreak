...
public class RoomAdPurchaseInfoMessageComposer extends MessageComposer {
    private final List<Room> rooms = new ArrayList<>();

    public RoomAdPurchaseInfoMessageComposer(List<Room> rooms) {
        for (Room room : rooms) {
            if (!room.isPromoted())
                this.rooms.add(room);
        }
    ...
    protected ServerMessage composeInternal() {
        this.response.init(Outgoing.RoomAdPurchaseInfoMessageComposer);
