...
public class PetFigureUpdateMessageComposer extends MessageComposer implements TIntObjectProcedure<Pet> {
    private final TIntObjectMap<Pet> pets;

    public PetFigureUpdateMessageComposer(Pet pet) {
        this.pets = new TIntObjectHashMap<>();
        this.pets.put(pet.getId(), pet);
    }

    public PetFigureUpdateMessageComposer(TIntObjectMap<Pet> pets) {
        this.pets = pets;
    }
