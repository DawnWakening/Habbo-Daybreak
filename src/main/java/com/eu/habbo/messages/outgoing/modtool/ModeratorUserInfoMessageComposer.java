...
public class ModeratorUserInfoMessageComposer extends MessageComposer {
    private static final Logger LOGGER = LoggerFactory.getLogger(ModeratorUserInfoMessageComposer.class);

    private final ResultSet set;

    public ModeratorUserInfoMessageComposer(ResultSet set) {
        this.set = set;
    }

    @Override
    protected ServerMessage composeInternal() {
        this.response.init(Outgoing.ModeratorUserInfoMessageComposer);
        try {
            int totalBans = 0;
